...
const pretestController = {
  all: async (req, res) => {
    try {
      ...
      const entry = await preTestV2.find(filter);
  ...
  paginate: async (req, res) => {
    ...
    try {
      ...
      const entry = await preTestV2.paginate(query, options);
  ...
  view: async (req, res) => {
    try {
      const entry = await preTestV2.findOne({ _id: req.params.id });
  ...
  create: async (req, res) => {
    try {
      ...
      //check if pretest with same lesson already exists
      const pretest = await preTestV2.findOne({ lessonId: data.lesson });
      if (pretest) throw "Pretest with same lesson already exists!";

      const entry = await preTestV2.create([
        {
          lessonId: data.lesson,
        },
      ]);

      if (!entry) throw "Pre test not created";
  ...
  //     const entry = await preTestV2.findOneAndUpdate(
  //       { _id: req.params.id },
  //       {
  //         name: data.name,
  //       },
  ...
  delete: async (req, res) => {
    try {
      ...
      const entry = await preTestV2.deleteOne({ _id: req.params.id });
